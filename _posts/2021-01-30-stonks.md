...
From there, what's the most commonly accepted, probably not accurate, but accurate enough metric by which "The Market" as a whole is typically measured? The famous S&P 500, of course. What does it actually represent, here in the year of our lord 20whatever? ¯\\\_(ツ)\_/¯ It means that when it goes up, a lot (or most) of the rest of the market does too. And since we don't actually have a true average of all the market activity for the day, the S&P is about as representative a real-time indicator of the whole damn thing as there exists. And that fits perfectly into our model of _extremely non-specific definition of good and bad, the details of which are so unimportant to you that you probably forgot what the question was while you were asking it._
...
- Free API usage
- Simple endpoint for current value of a symbol (SPY)
- Real-time-enough data
- ~~Limited to 8 API requests per minute; 800 API requests per day~~

> You'll be shocked to learn that this is now the third time I've had some free API provider decide to no longer be doing that, so I've switched yet again to something called [Alpaca](https://alpaca.markets) which I'm sure will now last forever. The code below has been updated to reflect the change.
{: .notice .author-note}
...
How do we turn some arbitrarily large number of requests for some piece of information into a set number that we control? Why, that's a cache, my friend. We can create some static resource to be the only thing our frontend knows about, and have a separate server-side task to update the contents of that resource on our own schedule. The dumb website can make as many requests as it likes to the resource to get the current value without causing any downstream API calls. All our stonksman needs to know is the current precent change in value of the S&P from the previous day's close. So our static resource can be an incredibly simple bit of json:
...
We don't care in the slightest about the existing state of things, so this is a prime opportunity for a Lambda script. We can just assume that every time it's triggered, it's an atomic operation to ask the API what's up with the S&P right now and write the relevant parts into our static json file in S3, leaving the frequency and timing of these updates to the scheduler. The bundled `boto3` library makes this an almost trivial python script.
...
We get yesterday's close and today's "close" (which is the current if the market is still open), and get the percent change between the two.
...
Looking back at this now, maybe I was a bit naive with the ranges I chose, but back in the quaint days of 2019, a 1.5% single day change in value *was* a pretty uncommon event! 
...
Ah, well after all, hindsight is 2020. Which is to say: a waking nightmare.